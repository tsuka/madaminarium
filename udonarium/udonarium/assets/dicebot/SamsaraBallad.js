...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$setPrefixes', '$debug', '$new', '$parse', '$==', '$command', '$roll', '$sort', '$map', '$+', '$*', '$[]', '$compare', '$compact', '$cmp_op', '$target_number', '$to_s', '$join', '$private', '$include?', '$!', '$send', '$fumble?', '$fumble', '$critical?', '$critical', '$<=', '$%', '$>=']);
  
  self.$require("utils/command_parser");
  return (function($base, $super, $parent_nesting) {
    var self = $klass($base, $super, 'SamsaraBallad');

    var $nesting = [self].concat($parent_nesting), $SamsaraBallad_rollDiceCommand$1, $SamsaraBallad_compare$3, $SamsaraBallad_fumble$ques$4, $SamsaraBallad_critical$ques$5;
    ...
    self.$setPrefixes(["SBS?.*"]);
    
    Opal.def(self, '$rollDiceCommand', $SamsaraBallad_rollDiceCommand$1 = function $$rollDiceCommand(command) {
      var $a, $b, $$2, self = this, parser = nil, cmd = nil, places_text = nil, total = nil, a = nil, b = nil, places = nil, cmp_result = nil, sequence = nil;

      
      self.$debug("rollDiceCommand Begin");
      parser = $$($nesting, 'CommandParser').$new("SB", "SBS");
      ...
      if (cmd.$command()['$==']("SB")) {
        ...
        $b = self.$roll(1, 10), $a = Opal.to_ary($b), (a = ($a[0] == null ? nil : $a[0])), $b;
        $b = self.$roll(1, 10), $a = Opal.to_ary($b), (b = ($a[0] == null ? nil : $a[0])), $b;
        places_text = "" + (a) + "," + (b);
        places = $send([a, b], 'map', [], ($$2 = function(n){var self = $$2.$$s || this;
    ...
    }, $SamsaraBallad_rollDiceCommand$1.$$arity = 1);
    self.$private();
    ...
    Opal.def(self, '$compare', $SamsaraBallad_compare$3 = function $$compare(total, cmd) {
      ...
      if ($truthy(["<=", "<"]['$include?'](cmd.$cmp_op()))) {
        if ($truthy(total.$send(cmd.$cmp_op(), cmd.$target_number())['$!']())) {
          return "失敗"
        } else if ($truthy(self['$fumble?'](total, cmd.$fumble()))) {
          return "ファンブル"
        } else if ($truthy(self['$critical?'](total, cmd.$critical()))) {
          return "クリティカル"
        } else {
          return "成功"
        }
    ...
    Opal.def(self, '$fumble?', $SamsaraBallad_fumble$ques$4 = function(total, fumble) {
      var $a, self = this;

      return ($truthy($a = fumble) ? $rb_le(total['$%'](10), fumble) : $a)
    }, $SamsaraBallad_fumble$ques$4.$$arity = 2);
    return (Opal.def(self, '$critical?', $SamsaraBallad_critical$ques$5 = function(total, critical) {
      var $a, self = this;

      return ($truthy($a = critical) ? $rb_ge(total['$%'](10), critical) : $a)
    }, $SamsaraBallad_critical$ques$5.$$arity = 2), nil) && 'critical?';
