...
(function(Opal) {
  ...
  Opal.add_stubs(['$amadeusDice', '$nil?', '$[]', '$upcase', '$===', '$get_table_by_1d6', '$get_table_by_2d6', '$get_table_by_d66_swap', '$=~', '$last_match', '$|', '$to_i', '$parren_killer', '$+', '$roll', '$collect', '$split', '$==', '$min', '$each', '$>', '$length', '$check_success', '$>=', '$check_hit', '$bcdice', '$freeze', '$setPrefixes', '$keys']);
  return (function($base, $super, $parent_nesting) {
    var self = $klass($base, $super, 'Amadeus_Korean');

    var $nesting = [self].concat($parent_nesting), $Amadeus_Korean_initialize$1, $Amadeus_Korean_rollDiceCommand$2, $Amadeus_Korean_amadeusDice$3, $Amadeus_Korean_check_success$6;

    
    Opal.const_set($nesting[0], 'ID', "Amadeus:Korean");
    Opal.const_set($nesting[0], 'NAME', "아마데우스");
    Opal.const_set($nesting[0], 'SORT_KEY', "国際化:Korean:아마데우스");
    Opal.const_set($nesting[0], 'HELP_MESSAGE', "" + "・판정(Rx±y@z>=t)\n" + "　다이스별로 성공, 실패의 판정을 합니다.\n" + "　x：x에 랭크(S,A～D)를 입력.\n" + "　y：y에 수정치를 입력. ±의 계산에 대응. 생략가능.\n" + "　z：z에 스페셜이 되는 주사위 눈의 최저치를 입력.\n" + "　생략한 경우, 6의 값만 스페셜이 됩니다.\n" + "　t：t에 목표치를 입력. ±의 계산에 대응. 생략가능.\n" + "　목표치를 생략한 경우, 목표치는 4로 계산됩니다.\n" + "　예） RA　RB-1　RC>=5　RD+2　RS-1@5>=6\n" + "　※ RB++ 나 RA- 같은, 플러스 마이너스만의 표기로는 계산되지 않습니다.\n" + "　\"달성치\"_\"판정결과\"[\"주사위 눈\"\"대응하는 인과\"]와 같이 출력됩니다.\n" + "　단, C.D랭크에서는 대응하는 인과가 출력되지 않습니다.\n" + "　출력예)　1_펌블！[1흑] / 3_실패[3청]\n" + "・각종표\n" + "　・조우표　ECT／관계표　RT／부모마음표　PRT／전장표　BST／휴식표　BT／\n" + "　　펌블표　FT／치명상표　FWT／전과표 BRT／랜덤아이템표　RIT／\n" + "　　손상표　WT／악몽표　NMT／목표표　TGT／制約表 CST\n" + "・試練表（～VT）\n" + "　ギリシャ神群 GCVT／ヤマト神群 YCVT／エジプト神群 ECVT／\n" + "　クトゥルフ神群 CCVT／北欧神群 NCVT／ダンジョン DGVT／日常 DAVT\n" + "・挑戦テーマ表（～CT）\n" + "　武勇 PRCT／技術 TCCT／頭脳 INCT／霊力 PSCT／愛 LVCT／日常 DACT\n");
    
    Opal.def(self, '$initialize', $Amadeus_Korean_initialize$1 = function $$initialize() {
      var $iter = $Amadeus_Korean_initialize$1.$$p, $yield = $iter || nil, self = this, $zuper = nil, $zuper_i = nil, $zuper_ii = nil;

      if ($iter) $Amadeus_Korean_initialize$1.$$p = null;
      // Prepare super implicit arguments
      for($zuper_i = 0, $zuper_ii = arguments.length, $zuper = new Array($zuper_ii); $zuper_i < $zuper_ii; $zuper_i++) {
        $zuper[$zuper_i] = arguments[$zuper_i];
      }
      
      $send(self, Opal.find_super_dispatcher(self, 'initialize', $Amadeus_Korean_initialize$1, false), $zuper, $iter);
      self.sendMode = 2;
      self.sortType = 1;
      return (self.d66Type = 2);
    }, $Amadeus_Korean_initialize$1.$$arity = 0);
    
    Opal.def(self, '$rollDiceCommand', $Amadeus_Korean_rollDiceCommand$2 = function $$rollDiceCommand(command) {
    ...
    }, $Amadeus_Korean_rollDiceCommand$2.$$arity = 1);
    
    Opal.def(self, '$amadeusDice', $Amadeus_Korean_amadeusDice$3 = function $$amadeusDice(command) {
      var $a, $b, $$4, $$5, self = this, commandText = nil, skillRank = nil, modifyText = nil, signOfInequality = nil, targetText = nil, specialNum = nil, diceCount = nil, modify = nil, target = nil, _ = nil, diceText = nil, diceList = nil, specialText = nil, message = nil, is_loop = nil;
      ...
      is_loop = false;
      $send(diceList, 'each', [], ($$5 = function(dice){var self = $$5.$$s || this, achieve = nil, result = nil;
        ...
        if ($truthy(is_loop)) {
          return (message = $rb_plus(message, "" + (achieve) + "_" + (result) + "[" + (dice) + ($$($nesting, 'INGA_TABLE')['$[]'](dice)) + "]"))
        } else {
          return (message = $rb_plus(message, "" + (achieve) + "_" + (result) + "[" + (dice) + "]"))
        };}, $$5.$$s = self, $$5.$$arity = 1, $$5));
      return message;
    }, $Amadeus_Korean_amadeusDice$3.$$arity = 1);
    
    Opal.def(self, '$check_success', $Amadeus_Korean_check_success$6 = function $$check_success(total_n, dice_n, signOfInequality, diff, special_n) {
